...
//        }
        ...
        Given("DEL_1_아래와 같이 특정 장소가 등록되어 있음") { dataTable: DataTable ->
...
//            assertEquals(expected, actual)
        ...
        And("DEL_2_아래와 같이 특정 유저가 등록되어 있음") { dataTable: DataTable ->
...
//            assertEquals(expected, actual)
        ...
        Given("DEL_3_유저가 아래와 같이 특정 장소에 대해 리뷰를 작성하였음") { dataTable: DataTable ->
...
//            assertEquals(reviewCounts, 0)
        ...
        And("DEL_4_리뷰 작성에 대한 보상으로 아래와 같이 유저에게 포인트가 부여되었음") { dataTable: DataTable ->
...
//            AddReviewActionHandler(eventDatabase).handleEvent(reviewEventDto)
        ...
        Given("DEL_5_유저의 현재 포인트 총점은 아래와 같음") { dataTable: DataTable ->
...
//            assertEquals(dataTable, "")
        ...
        When("DEL_6_유저가 아래와 같이 작성했던 리뷰를 삭제함") { dataTable: DataTable ->
            val (userId, rewardPoint) = dataTable.row(1)
//            assertEquals(userId, "")
        }

        Then("DEL_7_유저의 리워드 레코드가 아래와 같이 변경됨") { dataTable: DataTable ->
//            val review: Map<String, String> = dataTable.asMaps()[0]
//            val reviewId = review.get("reviewId")!!
//            val placeId = review.get("placeId")!!
//            val content = review.get("content")!!
...
//            assertEquals(dataTable, "")
        ...
        And("DEL_8_유저의 포인트 총점이 아래와 같아짐") { dataTable: DataTable ->
//            val review: Map<String, String> = dataTable.asMaps()[0]
//            val reviewId = review.get("reviewId")!!
//            val placeId = review.get("placeId")!!
//            val content = review.get("content")!!
...
//            assertEquals(dataTable, "")
        ...
        And("DEL_9_아래의 식별자를 가진 유저의 리뷰 레코드가 삭제됨") { dataTable: DataTable ->
//            val review: Map<String, String> = dataTable.asMaps()[0]
//            val reviewId = review.get("reviewId")!!
//            val placeId = review.get("placeId")!!
//            val content = review.get("content")!!
