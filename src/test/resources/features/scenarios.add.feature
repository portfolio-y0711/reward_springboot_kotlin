...
#  Rule: 유저가 작성한 글이 특정 장소에 대한 첫번째 리뷰글이면 유저에게 포인트가 부여됨
    ...
    Scenario: 사용자가 리뷰를 새로 작성함
      ...
      Given 3_아래 장소에 대한 리뷰글이 존재하지 않음
        | placeId                              |
        | 2e4baf1c-5acb-4efb-a1af-eddada31b00f |

      When 4_유저가 아래와 같이 리뷰글을 작성함
      ...
      Then 5_유저의 리워드 레코드가 아래와 같이 생성됨
      ...
      And 6_유저의 포인트 총점이 아래와 같아짐
        | userId                                | rewardPoint |
        | 3ede0ef2-92b7-4817-a5f3-0c575361f745  | 3           |

      And 7_유저의 리뷰 레코드가 아래와 같이 생성됨
